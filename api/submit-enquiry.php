...
if ($name === '' || mb_strlen($name) > 100 || !preg_match('/^\+?[0-9\s-]{8,15}$/', $phone) || ($email !== '' && !filter_var($email, FILTER_VALIDATE_EMAIL)) || !in_array($interest, $allowed, true)) { http_response_code(422); echo json_encode(['success' => false, 'message' => 'Please provide valid enquiry details.']); exit; }
...
try {
    ...
    $statement = database()->prepare('INSERT INTO enquiries (name, phone, email, interest, preferred_date, message) VALUES (:name, :phone, :email, :interest, :preferred_date, :message)');
    $statement->execute([':name' => $name, ':phone' => $phone, ':email' => $email !== '' ? $email : null, ':interest' => $interest, ':preferred_date' => $preferredDate !== '' ? $preferredDate : null, ':message' => $message !== '' ? $message : null]);
    require_once dirname(__DIR__) . '/notifications.php';
    notifyCourseEnquiry(['name' => $name, 'phone' => $phone, 'email' => $email, 'interest' => $interest, 'preferred_date' => $preferredDate, 'message' => $message]);
